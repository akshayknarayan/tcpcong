...
static inline bool check_ip(__u8 flow_addr[16], __u8 cfg_addr[16]) {
    __u64 *flow_addr_parts = (__u64*) flow_addr;
    __u64 *cfg_addr_parts = (__u64*) cfg_addr;

    return ((flow_addr_parts[0] & cfg_addr_parts[0]) == flow_addr_parts[0]) &&
           ((flow_addr_parts[1] & cfg_addr_parts[1]) == flow_addr_parts[1]);
}
...
int tracepoint_tcp_probe(struct tcp_probe_args *args) {
    ...
    if (cfg_f != NULL && 
        //cfg_f->family == f.family &&
        //check_ip(f.saddr, cfg_f->saddr) &&
        //check_ip(f.daddr, cfg_f->daddr) &&
